...
# ===============================
# 配置区
FOFA_EMAIL = "66840936@qq.com"          # 请填写您的 FOFA 邮箱（若不用 API 留空）
FOFA_KEY = "1a38ed4d7f6415ca1618409dad8f867d"            # 请填写您的 FOFA API Key（若不用 API 留空）
QUERY_BASE64 = "InVkcHh5IiAmJiBjb3VudHJ5PSJDTiI%3D"   # 同原查询
SIZE = 10000             # 每次拉取的最大条数
...
# ===============================
# 新增：FOFA API 获取 IP 列表
def fetch_fofa_ips():
    """调用 FOFA API 返回 IP:PORT 列表"""
    if not FOFA_EMAIL or not FOFA_KEY:
        print("ℹ️ 未设置 FOFA_EMAIL 或 FOFA_KEY，跳过 API 获取")
        return []
    url = f"https://fofa.info/api/v1/search/all?email={FOFA_EMAIL}&key={FOFA_KEY}&qbase64={QUERY_BASE64}&size={SIZE}&fields=host"
    try:
        ...
        ip_list = [item for item in results if item]
        print(f"📡 FOFA API 返回 {len(ip_list)} 个 IP:PORT")
        return ip_list
    except Exception as e:
...
def first_stage():
    ...
    # 1. 尝试通过 API 获取
    api_ips = fetch_fofa_ips()
    if api_ips:
        all_ips.update(api_ips)
        print(f"✅ 通过 API 获取到 {len(api_ips)} 个 IP")
    else:
        print("⚠️ API 未返回数据，尝试网页爬取...")
        # 2. 回退到原有的网页爬取方式
...
if __name__ == "__main__":
    ...
    push_all_files()    "CCTV8K": ["CCTV8K超高清", "CCTV-8K超高清", "CCTV-8K 超高清", "CCTV 8K"],
    "兵器科技": ["CCTV-兵器科技", "CCTV兵器科技"],
    "风云音乐": ["CCTV-风云音乐", "CCTV风云音乐"],
    "第一剧场": ["CCTV-第一剧场", "CCTV第一剧场"],
    "风云足球": ["CCTV-风云足球", "CCTV风云足球"],
...
def fetch_quake_ips():
    """调用360 Quake API，返回 IP:PORT 列表"""
    if not QUAKE_TOKEN:
        print("❌ 未设置 QUAKE_TOKEN，无法调用 API")
        return []
    ...
    payload = {
        ...
        "start": 0,
        "ignore_cache": False,
        "include": ["ip", "port", "service"]  # 确保返回端口
    }
    ...
    try:
        ...
        if data.get("code") != 0:
            ...
            print(f"❌ Quake API 错误 (code={data.get('code')}): {err_msg}")
            return []
        ...
        print(f"📡 Quake API 返回 {len(ip_list)} 个 IP:PORT")
        return ip_list

    except requests.exceptions.RequestException as e:
        print(f"❌ Quake API 请求失败：{e}")
        return []
    except json.JSONDecodeError:
        print("❌ Quake API 返回非 JSON 格式")
        return []
...
def first_stage():
    ...
    if ip_list:
        ...
        print("⚠️ Quake 未返回 IP，尝试备用爬取...")
        # 2. 备用：网页爬取（如果配置了 FALLBACK_URLS）
        for url in FALLBACK_URLS:
            try:
                r = requests.get(url, headers=HEADERS, timeout=15)
                urls_all = re.findall(r'<a href="http://(.*?)"', r.text)
    ...
    if not all_ips:
        print("⚠️ 未获取到任何 IP，第一阶段结束")
        return get_run_count() + 1  # 计数仍增加
...
if __name__ == "__main__":
    ...
    push_all_files()    "央视文化精品": ["CCTV-央视文化精品", "CCTV央视文化精品", "CCTV文化精品", "CCTV-文化精品", "文化精品"],
    "卫生健康": ["CCTV-卫生健康", "CCTV卫生健康"],
    "电视指南": ["CCTV-电视指南", "CCTV电视指南"],
    "农林卫视": ["陕西农林卫视"],
    "三沙卫视": ["海南三沙卫视"],
...
if __name__ == "__main__":
    ...
    push_all_files()    "高尔夫网球": ["CCTV-高尔夫网球", "CCTV高尔夫网球", "CCTV央视高网", "CCTV-高尔夫·网球", "央视高网"],
    "央视文化精品": ["CCTV-央视文化精品", "CCTV央视文化精品", "CCTV文化精品", "CCTV-文化精品", "文化精品"],
    "卫生健康": ["CCTV-卫生健康", "CCTV卫生健康"],
    "电视指南": ["CCTV-电视指南", "CCTV电视指南"],
    "农林卫视": ["陕西农林卫视"],
...
def fetch_fofa_ips():
    """调用 FOFA API 返回 IP:PORT 列表"""
    if not FOFA_EMAIL or not FOFA_KEY:
        print("❌ 未设置 FOFA_EMAIL 或 FOFA_KEY，无法调用 API")
        return []
    url = f"https://fofa.info/api/v1/search/all?email={FOFA_EMAIL}&key={FOFA_KEY}&qbase64={QUERY_BASE64}&size={SIZE}&fields=host"
    try:
        resp = requests.get(url, timeout=15)
